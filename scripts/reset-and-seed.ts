// Deletes all participant data (every row in irb_consent and everything that
// cascades from it: pre_task, task, post_task_vm, specific_meeting,
// demographics) and generates a fresh set of synthetic participants using
// the app's own consultant-matching logic, so the new data is internally
// consistent (agreement %s, strong/weak picks, etc. derive the same way the
// real flow produces them). survey_rec (the static 10-suggestion bank) is
// left untouched — it isn't participant data.
//
// Run with: npx tsx scripts/reset-and-seed.ts [count]
//
...
// 2-letter uppercase initials, matching the .toUpperCase()-forced input in
// SpecificMeetingQuestions.tsx (placeholders there are "CD"/"RY").
...
// DemographicsSurvey.tsx's `industry` is free text; this is just a plausible
// sample pool, not an enforced set of options.
...
async function deleteAllParticipantData() {
  console.log("Deleting all participant data...");
  // Everything else FKs to irb_consent with `on delete cascade`, so deleting
  // every row there clears pre_task, task, post_task_vm, specific_meeting,
  // and demographics too. survey_rec is untouched (not participant data).
  const { error, count } = await supabase
    .from("irb_consent")
    .delete({ count: "exact" })
    .not("participant_id", "is", null);
  if (error) throw error;
  console.log(`Deleted ${count ?? 0} participants (cascaded to all linked tables).`);
}
    ...
    await supabase.from("specific_meeting").upsert(
      {
        ...
        attendee1: randomInitials(),
        attendee2: randomInitials(),
...
async function main() {
  await deleteAllParticipantData();
  await seed();
}
